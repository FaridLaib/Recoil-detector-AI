...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(os.path.join(os.path.dirname(__file__), '..', 'logs', 'anti_recoil_log.txt')), logging.StreamHandler()]
)
...
class AntiRecoil:
    ...
    def apply_anti_recoil(self):
        ...
        for i in range(1, len(self.pattern)):
            prev_x, prev_y = self.pattern[i - 1]
            curr_x, curr_y = self.pattern[i]
            dx = curr_x - prev_x
            dy = curr_y - prev_y
            deltas.append([-dx, +dy])

        logging.info("Starting anti-recoil")
        ...
        for i, (dx, dy) in enumerate(deltas):
            ...
            x_stick = np.clip(dx / 100.0, -1.0, 1.0)
            y_stick = np.clip(dy / 100.0, -1.0, 1.0)
